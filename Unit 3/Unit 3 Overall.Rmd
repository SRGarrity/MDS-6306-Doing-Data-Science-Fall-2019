...
```{r}
# arrange is synonymous with sort
mpg %>% 
...
## mutate
```{r}
#mutat allows us to create a new variable (column in a data frame)
...
mpg %>% ggplot(aes(x=class, fill=class)) +
  geom_bar() +
  geom_text(aes(x=class, y=totals$n+2, label=totals$n, fill=NULL), data=totals)
```

## EDA Part X
```{r}
mpg %>% ggplot(aes(x=cty, fill=class)) +
  geom_histogram() +
  facet_grid(rows=vars(class))

mpg %>% group_by(class) %>%
  summarize(mean=mean(cty), sd=sd(cty), median=median(cty), range=max(cty)-min(cty),
            IQR=IQR(cty), count=n())
```
...
#Fifa
gg_miss_var(fifa[,1:40]) + ylim(0,80)

gg_miss_var(fifa[,41:89]) + ylim(0,80)

#mpg

#two ways to get the same info: 1) gg_miss_var vs. sapply:
