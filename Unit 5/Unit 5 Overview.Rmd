...
str_view(string1, "^Don't") #^ looks at beginning of the string
str_view(string1, "^interfere")
str_view(string3, "^T")
str_view(string6, "^\"Some") #Why Nothing ... we will come back to this.
str_view(string6, "Jobs$") #$ looks at end of the string

#Find digits and whitespace
str_view(string5,"\\d") # looks for a digit
...
str_view_all(df$quotes,"[^abc]") # anything NOT abc
```
...
unlist(str_split(s2,boundary("word")))
```
...
ArticleToClassify = allNYTSearch[22,] # Check out 4, 9 and 44
...
numNewsArticles = dim(newsArticles)[1] #number of rows
numOtherArticles = dim(otherArticles)[1] #number of rows
